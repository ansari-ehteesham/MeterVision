...
from metervision.pipeline.predictor import PredictionPipeline
    ...
    if pred_upload:
        ...
        if st.button("Start Prediction", type="primary"):
            ...
            with st.spinner("Wait For It...", show_time=True):
                ...
                display_arr, reading_arr = prediction_pipeline.prediction(img_array=img_arr)
            with st.container():
                img_col, roi_cols = st.columns(spec=2, gap="small", border=True)

                if img_arr.any() and display_arr.any():
                    with img_col:
                        st.image(image=img_arr, caption="Meter Image")
                    with roi_cols:
                        st.image(image=display_arr, caption="ROI Image", width=300)
                        st.image(image=reading_arr, caption="ROI Image", width=300)
            with st.container():
